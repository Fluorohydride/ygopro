...
int32 field::process_point_event(int16 step, int32 skip_trigger, int32 skip_freechain, int32 skip_new) {
	...
	case 2: {
		...
		for (auto clit = core.new_fchain_s.begin(); clit != core.new_fchain_s.end(); ++clit) {
			...
			effect* preason_effect = peffect->handler->current.reason_effect;
			bool act = true;
			if(peffect->is_chainable(tp) && peffect->is_activateable(tp, clit->evt, TRUE)
			        && (((peffect->code == EVENT_FLIP) && preason_effect && (preason_effect->flag & EFFECT_FLAG_EHREN)) || (clit->triggering_location & 0x3) 
