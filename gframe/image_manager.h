...
private:
	...
	using downloading_map = std::map<int/*code*/, std::pair<int /*status: value not found: not yet downloaded, 0: downloading, 1: not correctly downloaded, 2: correctly downloaded*/, path_string>>;
public:
	ImageManager() {
	...
	irr::video::IImage* GetTextureFromFile(const io::path& file, int width, int height, chrono_time timestamp_id, std::atomic<chrono_time>& source_timestamp_id, irr::io::IReadFile* archivefile = nullptr);
