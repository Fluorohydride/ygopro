...
bool MenuHandler::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			...
			case BUTTON_JOIN_SRVPRO: {
				bot_mode = false;
				srvpro_mode = true;
				char ip[20];
				int sel = mainGame->cbSrvProServer->getSelected();
				const wchar_t* pstr = mainGame->srvproInfo[sel].host;
				BufferIO::CopyWStr(pstr, ip, 16);
				unsigned int remote_addr = htonl(inet_addr(ip));
				if(remote_addr == -1) {
					char hostname[100];
					char port[6];
					BufferIO::CopyWStr(pstr, hostname, 100);
					BufferIO::CopyWStr(mainGame->srvproInfo[sel].port, port, 6);
					struct evutil_addrinfo hints;
					struct evutil_addrinfo *answer = NULL;
					memset(&hints, 0, sizeof(hints));
					hints.ai_family = AF_INET;
				...
				unsigned int remote_port = _wtoi(mainGame->srvproInfo[sel].port);
				BufferIO::CopyWStr(pstr, mainGame->gameConf.lasthost, 100);
				BufferIO::CopyWStr(mainGame->srvproInfo[sel].port, mainGame->gameConf.lastport, 20);
				if(DuelClient::StartClient(remote_addr, remote_port, false)) {
					mainGame->btnCreateHost->setEnabled(false);
					mainGame->btnJoinHost->setEnabled(false);
					mainGame->btnJoinCancel->setEnabled(false);
