...
struct SGUITTGlyph {
	//! Constructor.
	SGUITTGlyph() : isLoaded(false), glyph_page(0), advance({}), surface(nullptr), parent(nullptr) {}
...
public:
	CGUITTGlyphPage(video::IVideoDriver* Driver, const io::path& texture_name) : texture(nullptr), available_slots(0), used_slots(0), dirty(false), driver(Driver), name(texture_name) {}
	~CGUITTGlyphPage() {
		if (texture) {
			if (driver)
...
public:
	...
	void draw(const core::stringw& text, const core::rect<s32>& position,
	                  ...
	                  const core::rect<s32>* clip = nullptr) override;
	...
	s32 getKerningWidth(const wchar_t* thisLetter = nullptr, const wchar_t* previousLetter = nullptr) const override;
	...
	(const wchar_t* text, scene::ISceneManager* smgr, scene::ISceneNode* parent = nullptr,
