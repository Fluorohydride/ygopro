...
void TagDuel::TPResult(DuelPlayer* dp, unsigned char tp) {
	...
	last_replay.Flush();
	auto load_single = [&](std::vector<code_pointer>& deck_container, uint8_t p, uint8_t location) {
	...
	auto load_tag = [&](std::vector<code_pointer>& deck_container, uint8_t p, uint8_t location) {
		last_replay.WriteInt32(deck_container.size(), false);
		for (auto cit = deck_container.rbegin(); cit != deck_container.rend(); ++cit) {
