...
int DuelClient::ClientAnalyze(char * msg, unsigned int len) {
	...
	case MSG_HINT: {
		...
		case HINT_OPSELECTED: {
			...
			mainGame->lstLog->insertItem(0, textBuffer, -1);
			mainGame->logParam.push_front(0);
			mainGame->gMutex.Lock();
		...
		case HINT_CODE: {
			...
			mainGame->lstLog->insertItem(0, textBuffer, -1);
			mainGame->logParam.push_front(data);
			mainGame->gMutex.Lock();
			mainGame->SetStaticText(mainGame->stACMessage, 310, mainGame->guiFont, textBuffer);
			mainGame->PopupElement(mainGame->wACMessage, 20);
			mainGame->gMutex.Unlock();
	...
	case MSG_CONFIRM_DECKTOP: {
		...
		for (int i = 0; i < count; ++i) {
			...
			mainGame->lstLog->insertItem(0, textBuffer, -1);
			mainGame->logParam.push_front(pcard->code);
			mainGame->gMutex.Unlock();
			float shift = -0.15f;
	...
	case MSG_CONFIRM_CARDS: {
		...
		for (int i = 0; i < count; ++i) {
			...
			mainGame->lstLog->insertItem(0, textBuffer, -1);
			mainGame->logParam.push_front(code);
			mainGame->gMutex.Unlock();
			if (l & 0x41) {
				if(count == 1) {
					float shift = -0.15f;
