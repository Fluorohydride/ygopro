...
//	btnTestMode = env->addButton(rect<s32>(10, 135, 270, 165), wMainMenu, BUTTON_TEST_MODE, dataManager.GetSysString(1203));
	...
	posY += 30;
	chkDontDrawSingleChain = env->addCheckBox(false, rect<s32>(posX, posY, posX + 260, posY + 25), tabHelper, CHECKBOX_DRAW_SINGLE_CHAIN, dataManager.GetSysString(1287));
	chkDontDrawSingleChain->setChecked(gameConf.dont_draw_single_chain != 0);
	posY += 30;
...
void Game::LoadConfig() {
	...
	gameConf.dont_draw_single_chain = 0;
	...
	while(fgets(linebuf, 256, fp)) {
		...
		} else if(!strcmp(strbuf, "dont_draw_single_chain")) {
			gameConf.dont_draw_single_chain = atoi(valbuf);
...
void Game::SaveConfig() {
	...
	fprintf(fp, "dont_draw_single_chain = %d\n", gameConf.dont_draw_single_chain);
