...
void WindBotPanel::UpdateDescription() {
	...
	if (bot.difficulty != 0)
		params << fmt::format(mainGame->globalHandlers->dataManager->GetSysString(2055), bot.difficulty);
	else
		params << mainGame->globalHandlers->dataManager->GetSysString(2056);
	params << L"\n";
	if (bot.masterRules.size()) {
		...
		params << fmt::format(mainGame->globalHandlers->dataManager->GetSysString(2057), mr.c_str());
		params << L"\n";
	}
