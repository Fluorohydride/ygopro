...
void Game::DrawMisc() {
	...
	if(dInfo.lp[0] >= _wtoi(mainGame->ebStartLP->getText()))
		driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(335, 12, 625, 28), recti(0, 0, 16, 16), 0, 0, true);
	else driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(335, 12, 335 + 290 * dInfo.lp[0] / _wtoi(mainGame->ebStartLP->getText()), 28), recti(0, 0, 16, 16), 0, 0, true);
	if(dInfo.lp[1] >= _wtoi(mainGame->ebStartLP->getText()))
		driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(696, 12, 986, 28), recti(0, 0, 16, 16), 0, 0, true);
	else driver->draw2DImage(imageManager.tLPBar, mainGame->Resize(986 - 290 * dInfo.lp[1] / _wtoi(mainGame->ebStartLP->getText()), 12, 986, 28), recti(0, 0, 16, 16), 0, 0, true);
	if(lpframe) {
	...
	for(int i = 0; i < 5; ++i) {
		...
		if(pcard && pcard->code != 0) {
			...
			if (pcard->level!=0 && pcard->rank!=0) {
				adFont->draw(L"/", mainGame->Resize(m2 - 4, 356, m2 + 4, 366), 0xff000000, true, false, 0);
				adFont->draw(L"/", mainGame->Resize(m2 - 3, 357, m2 + 5, 367), 0xffffffff, true, false, 0);
				int w2 = adFont->getDimension(pcard->lvstring).Width;
				adFont->draw(pcard->lvstring, mainGame->Resize(m2 - 5, 356, m2 - 5, 366, -w2, 0, 0, 0), 0xff000000, false, false, 0);
				adFont->draw(pcard->lvstring, mainGame->Resize(m2 - 4, 357, m2 - 4, 367, -w2, 0, 0, 0), (pcard->type & TYPE_TUNER) ? 0xffffff00 : 0xffffffff, false, false, 0);
				adFont->draw(pcard->rkstring, mainGame->Resize(m2 + 4, 356, m2 + 4 + w2, 366), 0xff000000, false, false, 0);
				adFont->draw(pcard->rkstring, mainGame->Resize(m2 + 5, 357, m2 + 5 + w2, 367), 0xffff80ff, false, false, 0);
			}else if (pcard->rank != 0) {
				adFont->draw(pcard->rkstring, mainGame->Resize(473 + i * 80, 356, 475 + i * 80, 366), 0xff000000, false, false, 0);
				adFont->draw(pcard->rkstring, mainGame->Resize(474 + i * 80, 357, 476 + i * 80, 367),0xffff80ff, false, false, 0);
			}else if (pcard->level != 0) {
				adFont->draw(pcard->lvstring, mainGame->Resize(473 + i * 80, 356, 475 + i * 80, 366), 0xff000000, false, false, 0);
				adFont->draw(pcard->lvstring, mainGame->Resize(474 + i * 80, 357, 476 + i * 80, 367),(pcard->type & TYPE_TUNER) ? 0xffffff00 : 0xffffffff, false, false, 0);
			}
		}
	}
	for(int i = 0; i < 5; ++i) {
		...
		if(pcard && (pcard->position & POS_FACEUP)) {
			...
			adFont->draw(pcard->defstring, mainGame->Resize(m + 5, 236, m + 5 + w, 256),
			             pcard->defense > pcard->base_defense ? 0xffffff00 : pcard->defense < pcard->base_defense ? 0xffff2090 : 0xffffffff , false, false, 0);
			if (pcard->level!=0 && pcard->rank!=0) {
				adFont->draw(L"/", mainGame->Resize(m2 - 4, 272, m2 + 4, 292), 0xff000000, true, false, 0);
				adFont->draw(L"/", mainGame->Resize(m2 - 3, 273, m2 + 5, 293), 0xffffffff, true, false, 0);
				int w2 = adFont->getDimension(pcard->lvstring).Width;
				adFont->draw(pcard->lvstring, mainGame->Resize(m2 - 5, 272, m2 - 5, 292, -w2, 0, 0, 0), 0xff000000, false, false, 0);
				adFont->draw(pcard->lvstring, mainGame->Resize(m2 - 4, 273, m2 - 5, 293, -w2, 0, 0, 0), (pcard->type & TYPE_TUNER) ? 0xffffff00 : 0xffffffff, false, false, 0);
				adFont->draw(pcard->rkstring, mainGame->Resize(m2 + 4, 272, m2 + 4 + w2, 292), 0xff000000, false, false, 0);
				adFont->draw(pcard->rkstring, mainGame->Resize(m2 + 5, 273, m2 + 5 + w2, 293), 0xffff80ff, false, false, 0);
			}else if (pcard->rank != 0) {
				adFont->draw(pcard->rkstring, mainGame->Resize(779 - i * 71, 272, 800 - i * 71, 292), 0xff000000, false, false, 0);
				adFont->draw(pcard->rkstring, mainGame->Resize(780 - i * 71, 273, 800 - i * 71, 293),0xffff80ff, false, false, 0);
			}else if (pcard->level != 0) {
				adFont->draw(pcard->lvstring, mainGame->Resize(779 - i * 71, 272, 800 - i * 71, 292), 0xff000000, false, false, 0);
				adFont->draw(pcard->lvstring, mainGame->Resize(780 - i * 71, 273, 800 - i * 71, 293),(pcard->type & TYPE_TUNER) ? 0xffffff00 : 0xffffffff, false, false, 0);
			}
		}
	}
