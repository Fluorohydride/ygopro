...
void ClientField::ShowSelectCard(bool buttonok, bool chain) {
	...
	for(size_t i = 0; i < ct; ++i) {
		...
		if(selectable_cards[i]->code)
			...
			mainGame->btnCardSelect[i]->setImage(imageManager.tCover[selectable_cards[i]->controler], rect<s32>(0, 0, CARD_IMG_WIDTH, CARD_IMG_HEIGHT));
...
void ClientField::ShowLocationCard() {
	...
	for(size_t i = 0; i < ct; ++i) {
		...
		if(display_cards[i]->code)
			...
			mainGame->btnCardDisplay[i]->setImage(imageManager.tCover[display_cards[i]->controler], rect<s32>(0, 0, CARD_IMG_WIDTH, CARD_IMG_HEIGHT));
