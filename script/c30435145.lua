...
function c30435145.filter(c,g)
	return c:GetLevel()>4 and c:IsSetCard(0x7) and g:CheckWithSumEqual(Card.GetLevel,c:GetLevel()*2,1)
end
...
function c30435145.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		local sg=rg:SelectWithSumEqual(tp,Card.GetLevel,tc:GetLevel()*2,1)
