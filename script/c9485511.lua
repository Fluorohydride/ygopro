...
function c9485511.initial_effect(c)
    ...
    e2:SetCondition(c9485511.con)
    e2:SetTarget(c9485511.tg)
    e2:SetOperation(c9485511.op)
    c:RegisterEffect(e2)
end
...
function c9485511.filter(c)
    return c:IsSetCard(0xb2) and c:IsFaceup()
end
function c9485511.con(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if c:IsStatus(STATUS_BATTLE_DESTROYED) then return false end
    return Duel.GetTurnPlayer()~=tp
end
function c9485511.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return chkc:IsControler(tp) and chkc:IsOnField() and c9485511.filter(chkc) end
...
function c9485511.op(e,tp,eg,ep,ev,re,r,rp)
