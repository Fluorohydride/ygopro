...
function c1264319.filter2(c,e,tp,m,chkf)
	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c1264319.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,chkf)
			end
		end
		return res
	end
...
function c1264319.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		local fgroup=ce:GetTarget()
		mg2=fgroup(ce,e,tp)
		sg2=Duel.GetMatchingGroup(c1264319.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,chkf)
	end
