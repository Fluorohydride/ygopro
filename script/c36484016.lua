...
function c36484016.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(0x101) and (not f or f(c))
		...
		and m:IsExists(Card.IsType,1,nil,TYPE_SYNCHRO)
end
...
function c36484016.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			local smat1=mg1:FilterSelect(tp,Card.IsType,1,1,nil,TYPE_SYNCHRO)
			mg1:RemoveCard(smat1:GetFirst())
			local mat1=Duel.SelectFusionMaterial(tp,tc,mg1,smat1:GetFirst(),chkf)
			tc:SetMaterial(mat1)
			Duel.Remove(mat1,POS_FACEUP,REASON_EFFECT+REASON_MATERIAL+REASON_FUSION)
			Duel.BreakEffect()
			Duel.SpecialSummon(tc,SUMMON_TYPE_FUSION,tp,tp,false,false,POS_FACEUP)
		else
			local smat2=mg2:FilterSelect(tp,Card.IsType,1,1,nil,TYPE_SYNCHRO)
			mg2:RemoveCard(smat2:GetFirst())
			local mat2=Duel.SelectFusionMaterial(tp,tc,mg2,smat2:GetFirst(),chkf)
