...
function c45033006.filter(c)
	return c:IsFacedown() and c:IsDefence() and c:IsControlerCanBeChanged()
end
...
function c45033006.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFacedown() and tc:IsDefence() and not Duel.GetControl(tc,tp,PHASE_END,1) then
