...
function c3370104.distg(e,c)
	...
	return tc:IsControler(e:GetHandlerPlayer()) and tc:IsLocation(LOCATION_MZONE) and tc:IsRace(RACE_MACHINE)
end
function c3370104.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc:IsControler(tp) and tc:IsLocation(LOCATION_MZONE) and tc:IsRace(RACE_MACHINE) then
