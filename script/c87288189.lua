...
function c87288189.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if e:GetLabel()==1 then
		g=Duel.SelectTarget(tp,nil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,2,nil)
	else
		g=Duel.SelectTarget(tp,nil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
	end
...
function c87288189.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:IsLocation(LOCATION_REMOVED) and tc:IsType(TYPE_MONSTER) and tc:IsAttribute(ATTRIBUTE_DARK) then
			...
			local g1=Duel.GetMatchingGroup(Card.IsCode,p,0x53,0,nil,tc:GetCode())
			rg:Merge(g1)
		end
		tc=og:GetNext()
	end
