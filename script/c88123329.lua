...
function c88123329.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(88123329,0))
...
function c88123329.tgfilter(c,race)
	return c:IsSetCard(0xb5) and c:IsAbleToGrave() and c:IsRace(race)
end
...
function c88123329.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local rm=Duel.SelectMatchingCard(tp,c88123329.rmfilter,tp,LOCATION_EXTRA,0,1,1,nil,tp)
	if rm:GetCount()==0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local tg=Duel.SelectMatchingCard(tp,c88123329.tgfilter,tp,LOCATION_DECK,0,1,1,nil,rm:GetFirst():GetRace())
