...
function c83236601.initial_effect(c)
	...
	e2:SetOperation(c83236601.rum)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1,83236602)
	e3:SetCondition(c83236601.condition)
	e3:SetTarget(c83236601.target)
	e3:SetOperation(c83236601.activate)
	c:RegisterEffect(e3)
...
function c83236601.tgfilter(c,e,tp)
...
function c83236601.rum(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():RegisterFlagEffect(83236601,RESET_EVENT+0x1ec0000+RESET_PHASE+PHASE_END,0,1)
end
function c83236601.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(83236601)>0 and Duel.GetCurrentPhase()==PHASE_MAIN2
end
function c83236601.thfilter(c)
	return c:IsSetCard(0x95) and c:IsType(TYPE_QUICKPLAY) and c:IsAbleToHand()
end
function c83236601.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c83236601.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c83236601.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c83236601.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
