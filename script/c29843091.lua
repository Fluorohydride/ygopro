...
function c29843091.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(1-tp,LOCATION_MZONE)>2
		and Duel.IsPlayerCanSpecialSummonMonster(tp,0,0xf,0,1000,2,RACE_BEAST,ATTRIBUTE_LIGHT) end
...
function c29843091.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,0,0xf,0,1000,2,RACE_BEAST,ATTRIBUTE_LIGHT,POS_FACEUP_DEFENCE,1-tp) then return end
	for i=1,3 do
		local token=Duel.CreateToken(tp,29843091+i,0xf,0,1000,2,RACE_BEAST,ATTRIBUTE_LIGHT)
