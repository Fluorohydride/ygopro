...
function c36006208.filter(c)
    return c:IsSetCard(0xb1) and c:IsFaceup() and c:IsAbleToGrave()
end
function c36006208.costfilter(c,rg,dg)
    if not rg:IsContains(c) then return false end
    local a=0
...
function c36006208.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chk==0 then
        ...
        if e:GetLabel()==1 then
            ...
            return Duel.IsExistingMatchingCard(c36006208.costfilter,tp,LOCATION_MZONE,0,2,nil,rg,dg)
        else
    ...
    if e:GetLabel()==1 then
        ...
        local sg=Duel.SelectMatchingCard(tp,c36006208.costfilter,tp,LOCATION_MZONE,0,2,2,nil,rg,dg)
